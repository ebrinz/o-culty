"""CLI entry point for analysis: search, cluster, similarity, visualize."""
import argparse
import logging
...
def cmd_search(args, config):
    ...
    embed_cfg = config["embedding"]["search"]
    embedder = Embedder(
    ...
    results = semantic_search(
        ...
        top_k=args.top_k, filter_tradition=args.tradition,
    )
...
def cmd_similarity(args, config):
    ...
    for i, label in enumerate(labels):
        row = label[:25].ljust(25) + "  ".join(
            f"{matrix[i, j]:.3f}".ljust(12) for j in range(len(labels))
        )
        print(row)
...
def cmd_visualize(args, config):
    ...
    coords = reduce_dimensions(
        vectors, method=args.method, n_components=2,
...
def main():
    ...
    sp_search = subparsers.add_parser("search", help="Semantic search (uses BGE-en-ICL)")
    ...
    subparsers.add_parser("cluster", help="Cluster embeddings (uses nomic)")

    sp_sim = subparsers.add_parser("similarity", help="Similarity matrix (uses nomic)")
    sp_sim.add_argument("--level", choices=["tradition", "title"], default="tradition")

    sp_viz = subparsers.add_parser("visualize", help="UMAP/t-SNE visualization (uses nomic)")
